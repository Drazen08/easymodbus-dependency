...
import com.github.sunjx.modbus.common.RegisterOrder;
...
public class Registers {
    ...
    public void setHoldingRegister(int offset, float[] values) {
        ...
        if (validateOffset(offset)) {
            ...
            for (int i = 0; i < values.length; i++) {
                /*  38 */
                int[] vv = RegistersUtil.convertFloatToRegisters(values[i], RegisterOrder.HighLow);
                int arrayOfInt[], j;
                byte b;
                /*  39 */
                for (arrayOfInt = vv, j = arrayOfInt.length, b = 0; b < j; ) {
                    Integer v = Integer.valueOf(arrayOfInt[b]);
                    /*  40 */
                    list.add(v);
                    b++;
                }
            ...
            /*  43 */
            int[] registers = IntArrayUtil.toIntArray(list);
            /*  44 */
    ...
    public void setHoldingRegister(int offset, Integer... values) {
        ...
        if (validateOffset(offset)) {
            ...
            for (int i = 0; i < values.length; i++) {
                /*  52 */
                int[] vv = RegistersUtil.convertIntToRegisters(values[i].intValue(), RegisterOrder.HighLow);
                int arrayOfInt[], j;
                byte b;
                /*  53 */
                for (arrayOfInt = vv, j = arrayOfInt.length, b = 0; b < j; ) {
    ...
    protected void setHoldingRegister(int offset, int[] registers) {
        ...
        if (validateOffset(offset))
            ...
            /*  65 */
            setHoldingRegister(offset + i, (short) registers[i]);
    }
    ...
    public synchronized void setCoil(int offset, boolean value) {
        ...
        if (validateOffset(offset)) {
            /* 103 */
            this.coils.put(Integer.valueOf(offset), Boolean.valueOf(value));
        }
    }

    /* 107 */
    public boolean getCoil(int offset) {
        return getBoolean(offset, this.coils);
    }


    /* 111 */
    public boolean getInput(int offset) {
        return getBoolean(offset, this.inputs);
    }


    /* 115 */
    public short getInputRegister(int offset) {
        return getShort(offset, this.inputRegisters);
    }


    /* 119 */
    public float getInputRegister4Float(int offset) {
        return getRegister4Float(offset, this.inputRegisters);
    }


    /* 123 */
    public int getInputRegister4Int(int offset) {
        return getRegister4Int(offset, this.inputRegisters);
    }


    /* 127 */
    public short getHoldingRegister(int offset) {
        return getShort(offset, this.holdingRegisters);
    }


    /* 131 */
    public float getHoldingRegister4Float(int offset) {
        return getRegister4Float(offset, this.holdingRegisters);
    }


    /* 135 */
    public int getHoldingRegister4Int(int offset) {
        return getRegister4Int(offset, this.holdingRegisters);
    }


    /* 139 */
    public int[] getInputRegisters(int offset, int registersCount) {
        return getRegisters(offset, this.inputRegisters, registersCount);
    }


    /* 143 */
    public int[] getHoldingRegisters(int offset, int registersCount) {
        return getRegisters(offset, this.holdingRegisters, registersCount);
    }


    protected float getRegister4Float(int offset, Map<Integer, Short> map) {
        /* 147 */
        int[] registers = getRegisters(offset, map, 4);
        /* 148 */
        return RegistersUtil.convertRegistersToFloat(registers);
    }

    protected int getRegister4Int(int offset, Map<Integer, Short> map) {
        /* 152 */
        int[] registers = getRegisters(offset, map, 2);
        /* 153 */
        return RegistersUtil.convertRegistersToInt(registers);
    }

    protected int[] getRegisters(int offset, Map<Integer, Short> map, int registersCount) {
        /* 157 */
        int[] rs = new int[registersCount];
        /* 158 */
        for (int i = 0; i < registersCount; i++)
            /* 159 */
            rs[i] = getShort(offset + i, map);
        /* 160 */
        return rs;
    }
    ...
    protected short getShort(int offset, Map<Integer, Short> map) {
        /* 164 */
        Short value = map.get(Integer.valueOf(offset));
        /* 165 */
        if (value == null) {
            /* 166 */
            return 0;
    ...
    protected boolean getBoolean(int offset, Map<Integer, Boolean> map) {
        /* 172 */
        Boolean value = map.get(Integer.valueOf(offset));
        /* 173 */
        if (value == null) {
            /* 174 */
            return false;
