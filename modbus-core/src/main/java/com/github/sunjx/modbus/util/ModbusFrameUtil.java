...
/* 40 */   public static void showFrameLog(ChannelLogger log, Channel channel, ModbusFrame frame, boolean isShowHex) { log.debug(channel, getFrameLog(channel, frame, isShowHex), new Object[0]); }



/* 44 */   public static void showFrameLog(InternalLogger log, Channel channel, ModbusFrame frame, boolean isShowHex) { log.debug(getFrameLog(channel, frame, isShowHex)); }


   public static String getFrameLog(Channel channel, ModbusFrame frame, boolean isShowHex) {
/* 48 */     byte[] bytes = ByteBufUtil.toBytes(frame.encode());
/* 49 */     String v1 = HexUtil.bytesToHexString(bytes, " ");
/* 50 */     String v2 = frame.toString();

/* 52 */     boolean isResponseFrame = frame.getFunction().toString().contains("Response");
/* 53 */     int transactionId = frame.getHeader().getTransactionIdentifier();

/* 55 */     StringBuilder sBuilder = new StringBuilder();
/* 56 */     sBuilder.append(String.format("%s", new Object[] { getChannelAddress(channel) }));
/* 57 */     if (isShowHex) {
/* 58 */       sBuilder.append(String.format("\r\n%s", new Object[] { splitLine }));
/* 59 */       sBuilder.append(String.format("\r\n%s", new Object[] { v1 }));
     }
/* 61 */     sBuilder.append(String.format("\r\n%s", new Object[] { splitLine }));

/* 63 */     if (isResponseFrame) {
/* 64 */       sBuilder.append(String.format("\r\n%s %s", new Object[] { (transactionId < 0) ? "RTU" : "TCP", v2 }));
/* 65 */       sBuilder.append(String.format("\r\n%s", new Object[] { splitLine }));
/* 66 */       sBuilder.append(getFunctionLog(frame.getFunction()));
     } else {
/* 68 */       sBuilder.append(String.format("\r\n%s", new Object[] { v2 }));
/* 69 */       sBuilder.append(String.format("\r\n%s", new Object[] { splitLine }));
     }
